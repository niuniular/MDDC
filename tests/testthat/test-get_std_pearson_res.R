...
test_that("get_std_pearson_res works correctly", {
  ...
  dat_mat <- as.data.frame(dat_mat)  # Convert to dataframe for row and column names
  ...
  contin_table <- check_and_fix_contin_table(as.matrix(dat_mat))  # Ensure it's a matrix
