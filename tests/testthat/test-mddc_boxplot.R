test_that("test MDDC boxplot function", {
    ...
    test_results <- function(col_specific_cutoff, separate, if_col_cor) {
        ...
        expect_equal(dim(contin_table), dim(mddc_boxplot_res$boxplot_signal))
        expect_equal(dim(contin_table), dim(mddc_boxplot_res$corr_signal_pval))
        expect_equal(dim(contin_table), dim(mddc_boxplot_res$corr_signal_adj_pval))
    }
    ...
    mddc_boxplot_res <- mddc_boxplot(zero_table, col_specific_cutoff = TRUE, separate = TRUE, if_col_cor = TRUE)
    expect_type(mddc_boxplot_res, "list")
    ...
    mddc_boxplot_res <- mddc_boxplot(large_table, col_specific_cutoff = TRUE, separate = TRUE, if_col_cor = TRUE)
    expect_type(mddc_boxplot_res, "list")
